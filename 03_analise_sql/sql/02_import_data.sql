...
-- 2. Importar Despesas
TRUNCATE TABLE demonstracoes_financeiras;
...
LOAD DATA LOCAL INFILE 'C:/Users/nicol/Documentos/Arquivos - Estudo/Projetos/health-data-challenge-2025/data/processed/consolidado_despesas.csv'
    ...
    LINES TERMINATED BY '\r\n'
    IGNORE 1 LINES
    (@reg_ans, @dummy_razao, @trim_raw, @ano_raw, @valor_raw)
    SET
        registro_ans = @reg_ans,
        ano = @ano_raw,
        trimestre = REPLACE(@trim_raw, 'T', ''),
        data_referencia = STR_TO_DATE(CONCAT(@ano_raw, '-', (REPLACE(@trim_raw, 'T', '') * 3 - 2), '-01'), '%Y-%m-%d'),
        valor = CAST(REPLACE(@valor_raw, '\r', '') AS DECIMAL(18,2)),
